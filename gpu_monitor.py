...
def check_thresholds(gpu, temp_threshold=80, mem_threshold=90, power_threshold=95):
    ...
    if gpu['temperature'] and gpu['temperature'] > temp_threshold:
        alerts.append(f"GPU {gpu['index']}: Temperature {gpu['temperature']}°C exceeds {temp_threshold}°C")
    ...
    if gpu['power_draw'] and gpu['power_limit']:
        ...
        if power_percent > power_threshold:
            alerts.append(f"GPU {gpu['index']}: Power {power_percent:.1f}% exceeds {power_threshold}%")
...
def print_status(gpus):
    ...
    for gpu in gpus:
        ...
        power_str = f"{gpu['power_draw']:.0f}W/{gpu['power_limit']:.0f}W" if gpu['power_draw'] else "N/A"
